...
export default {
  setup () {
    const { getExpense, updateExpense } = ExpenseEventualServices()
    const expenseEventual = ref({})
    const router = useRoute()
    const redirect = useRouter()
    const swal = inject('$swal')
    getExpense(router.params.ExpenseId, data => {
        ...
        swal
          ...
          .then(result => {
            if (result.isConfirmed) {
              redirect.push('/GastosEventuales')
            }
          })
      })
    }
